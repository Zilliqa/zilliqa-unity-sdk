...
namespace Zilliqa.Core.Beta
{
    public class KeyStore
    {
        ...
        public byte[] GenerateDerivedScryptKey(string pwd, byte[] saltBytes)
        {
            //Using Scrypt.Net from nuget
            var encoder = new Scrypt.ScryptEncoder();
            //ScyptDec
            var key = encoder.Encode(pwd, saltBytes);
        ...
        public byte[] GetDerivedKeyWithSalt(byte[] password, KDFParams parameters)
        {
            if (parameters is Pbkdf2)
            {
                var pbkdf2Params = (Pbkdf2)parameters;
            ...
            else if (parameters is KDFParams)
            {
                ...
                return GenerateDerivedScryptKey(Encoding.UTF8.GetString(password), ByteUtil.HexStringToByteArray(parameters.Salt));
            }
            else
            {
                throw new Exception("unsupport kdf params");
        ...
        public string EncryptPrivateKey(string privateKey, string passphrase, KDFType type)
        {
            ...
            if (type == KDFType.PBKDF2)
            {
                ...
                derivedKey = GetDerivedKeyWithSalt(Encoding.Default.GetBytes(passphrase), pbkdf2Params);
            }
            else
            {
                ...
                scryptParams.r = 8;
                scryptParams.n = 8192;
                derivedKey = GetDerivedKeyWithSalt(Encoding.Default.GetBytes(passphrase), scryptParams);
            }
            ...
            var kp = new KDFParams() { Salt = ByteUtil.ByteArrayToHexString(salt) };
            ...
            Debug.Log("encryption derived key " + ByteUtil.ByteArrayToHexString(derivedKey));
        ...
        public string DecryptPrivateKey(string encryptJson, string passphrase)
        {
            ...
            Debug.Log("decryption derived key " + ByteUtil.ByteArrayToHexString(derivedKey));
