...
namespace Zilliqa.Marketplace
{
    ...
    public class Order : Dictionary<string, JObject>
    {
        ...
        public async Task<List<OrderInfo>> GetInfo(OrderType orderType, MarketplaceType marketplaceType)
        {
        ...
        private async Task<List<OrderInfo>> GetFixedPriceInfo(OrderType orderType)
        {
        ...
        private async Task<List<OrderInfo>> GetAuctionOrders(OrderType orderType)
        {
            var lastTxBlock = await ZilliqaRPC.GetLatestTxBlock();
